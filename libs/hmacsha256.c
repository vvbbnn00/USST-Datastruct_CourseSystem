...
char *calcHexHMACSHA256(const char *content_, const char *secret_) {
    ...
    unsigned long long len1 = strlen((char *) secret_);
    unsigned long long len2 = strlen((char *) content_);
    uint8_t secret[len1], indata[len2];
    ...
    char *FINAL_STR = malloc(65 * sizeof(char));
    memset(FINAL_STR, 0, 65 * sizeof(char));
